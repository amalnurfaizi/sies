...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT * FROM `pendaftar` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0 ){
    ...
    <form action="" id="application-form" class="py-3">
        ...
        <div class="form-group">
            ...
            <select name="status" id="status" class="form-select rounded-0" required>
                <option class="px-2 py-2" value="0" <?= isset($status) && $status == 0 ? 'selected': '' ?>>Tertunda</option>
                <option class="px-2 py-2" value="1" <?= isset($status) && $status == 1 ? 'selected': '' ?>>Terkonfirmasi</option>
                <option class="px-2 py-2" value="2" <?= isset($status) && $status == 2 ? 'selected': '' ?>>Disetujui</option>
                <option class="px-2 py-2" value="3" <?= isset($status) && $status == 3 ? 'selected': '' ?>>Ditolak</option>
            </select>
        </div>
    </form>
</div>
