<section class="py-4">
    <div class="container">
        <h3 class="fw-bolder text-center">List Anggota</h3>
        <center>
...
<script>
    $(function(){
        $('.dt-init').DataTable({
            ...
            searching: false,
            ajax: {
                url:"../classes/Master.php?f=data_anggota",
                method:"POST"
            },
            columns: [{
                    data: 'no',
                ...
                // },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        el.attr('id_anggota','dropdown'+row.id_anggota)
                        el.find('.dropdown-menu').attr('aria-labelledby','dropdown'+row.id_anggota)
                        el.find('.edit_data,.delete_data,.view_data').attr('data-id',row.id_anggota).attr('data-nama_lengkap',row.nama_lengkap)
                        el.find('.edit_data').attr("href","./?page=anggota/FormAnggota&id_anggota="+row.id_anggota)
                        el.find('.view_data').attr("href","./?page=anggota/DetailAnggota&id_anggota="+row.id_anggota)
            ...
            drawCallback: function(settings) {
                ...
                $('.delete_data').click(function(){
                    _conf("Apakah Yakin Ingin Menghapus Anggota Dengan Nama <b>"+$(this).attr('data-nama_lengkap')+"</b> Dari List ?","hapus_anggota",[$(this).attr('data-id')])
                })
            },
            language:{
                oPaginate: {
