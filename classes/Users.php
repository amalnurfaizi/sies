...
Class Users extends DBConnection {
	...
	public function simpanPengguna(){
		...
		$oid = $id;
		$data = '';
		$chk = $this->conn->query("SELECT * FROM `pengguna` where nama_pengguna ='{$nama_pengguna}' ".($id>0? " and id!= '{$id}' " : ""))->num_rows;
		if($chk > 0){
			return 3;
			exit;
		}
		...
		if($save){
			...
			if(empty($id))
				$this->settings->set_flashdata('success','Akun Berhasil Di Buat');
			else
				$this->settings->set_flashdata('success','Akun Berhasil Di Perbarui');
			$resp['status'] = 'success';
		...
		if(isset($_FILES['image']) && $_FILES['image']['tmp_name'] != ''){
			...
			if(isset($uploaded_img)){
				...
				if($uid == $this->settings->userdata('id')){
						$this->settings->set_userdata('foto_profil',$fname);
				}
			}
		}
