<section class="py-4">
    <div class="container">
        <h3 class="fw-bolder text-center">Daftar Pengguna</h3>
        <center>
...
<script>
    $(function(){
        $('.dt-init').DataTable({
            ...
            serverSide: true,
            ajax: {
                url:"../classes/Master.php?f=data_pengguna",
                method:"POST"
            },
            columns: [{
                    data: 'no',
                ...
                {
                    data: 'nama_lengkap',
                    className: 'nama_lengkap',
                    width:"25%"
                },
                {
                    data: 'nama_pengguna',
                    className: 'nama_pengguna',
                    width:"25%"
                },
                {
                    data: 'ekskul',
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        el.find('.edit_data,.delete_data,.view_data,.reset_pass').attr('data-id',row.id).attr('data-name',row.nama_lengkap)
                        el.find('.edit_data').attr("href","./?page=pengguna/FormPengguna&id="+row.id)
                        el.find('.view_data').attr("href","./?page=pengguna/DetailPengguna&id="+row.id)
            ...
            drawCallback: function(settings) {
                ...
                $('.delete_data').click(function(){
                    _conf("Apakah Yakin Menghapus Pengguna Dengan Nama <b>"+$(this).attr('data-name')+"</b> Dari list?","menghapus_pengguna",[$(this).attr('data-id')])
                })
                $('.reset_pass').click(function(){
                    _conf("Apakah Yakin Ingin Mereset Password <b>"+$(this).attr('data-name')+"</b> ?","reset_pass",[$(this).attr('data-id')])
                })
            },
            language:{
                oPaginate: {
    ...
    function menghapus_pengguna($id){
        start_loader();
        var _this = $(this)
        $('.err-msg').remove();
        var el = $('<div>')
        ...
        el.hide()
        $.ajax({
            url: '../classes/Master.php?f=menghapus_pengguna',
            method: 'POST',
            data: {
                id: $id
            },
