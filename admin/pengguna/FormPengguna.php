...
<section class="py-5">
    <div class="container">
        <h2 class="fw-bolder text-center"><b><?= isset($id) ? "Edit Pengguna" : "Tambah Pengguna" ?></b></h2>
        <hr>
            ...
            <div class="col-lg-5 col-md-10 col-sm-12 col-xs-12">
                <form action="" id="user-form" class="py-3">
                    ...
                    <div class="input-group mb-3 input-group-dynamic <?= isset($nama_depan) ? 'is-filled' : '' ?>">
                        ...
                        <input type="text" id="nama_depan" name="nama_depan" autofocus value="<?= isset($nama_depan) ? $nama_depan : "" ?>" class="form-control" required="required">
                    </div>
                    ...
                    <div class="input-group mb-3 input-group-dynamic <?= isset($nama_belakang) ? 'is-filled' : '' ?>">
                        ...
                        <input type="text" id="nama_belakang" name="nama_belakang" value="<?= isset($nama_belakang) ? $nama_belakang : "" ?>" class="form-control" required="required">
                    </div>
                    <div class="input-group mb-3 input-group-dynamic <?= isset($nama_pengguna) ? 'is-filled' : '' ?>">
                        ...
                        <input type="text" id="nama_pengguna" name="nama_pengguna" value="<?= isset($nama_pengguna) ? $nama_pengguna : "" ?>" class="form-control" required="required">
                    </div>
                    <div class="form-group mb-3">
                        <label for="tipe" class="form-label">Tipe <span class="text-danger">*</span></label>
                        <select name="tipe" id="tipe" class="form-select rounded-0" required>
                            <option class="px-2 py-2" value="1" <?= isset($tipe) && $tipe == 1 ? 'selected': '' ?>>Admin</option>
                            <option class="px-2 py-2" value="2" <?= isset($tipe) && $tipe == 2 ? 'selected': '' ?>>Pembina Ekskul</option>
                            <option class="px-2 py-2" value="3" <?= isset($tipe) && $tipe == 3 ? 'selected': '' ?>>Ketua Ekskul</option>
                        </select>
                    </div>
                    <div id="club-field" class="input-group mb-3 input-group-static is-filled <?= !isset($tipe) || (isset($tipe) && $tipe == 1) ? 'd-none': '' ?>">
                        <label for="id_ekskul" class="form-label">Ekskul</label>
                        <select id="id_ekskul" name="id_ekskul" class="form-select" <?= (isset($tipe) && $tipe == 2) ? '': '' ?>>
                        <option value="" <?= !isset($id_ekskul) ? "selected" : "" ?> disabled="disabled"></option>
                        <?php 
                        $ekskul = $conn->query("SELECT * FROM `ekskul` where logo = 0 and `status` = 1 order by `nama_ekskul` asc");
                    ...
                    <div class="row">
                        <div class="col-md-12 text-center">
                            ...
                            <a href="./?page=pengguna" class="btn bg-deafult border bg-gradient btn-sm w-25"><span class="material-icons">keyboard_arrow_left</span> Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    ...
    $(function(){
        ...
        $('#user-form').submit(function(e){
            ...
            start_loader()
            $.ajax({
                url:'../classes/Master.php?f=simpan_pengguna',
                type:'POST',
                method:'POST',
                cache:false,
                contentType:false,
