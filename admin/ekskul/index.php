...
<section class="py-4">
    <div class="container">
        <h3 class="fw-bolder text-center">Daftar Ekstrakurikuler</h3>
        <center>
...
<script>
    $(function(){
        $('.dt-init').DataTable({
            ...
            searching: false,
            ajax: {
                url:"../classes/Master.php?f=data_ekskul",
                method:"POST"
            },
            columns: [{
                    data: 'no',
                ...
                },
                {
                    ...
                    render:function(data, type, row, meta){
                        return '<p class="m-0 text-truncate w-100">'+((row.jadwal).substr(0,30))+'...</p>';
                        },
                    width:"30%"
                },
                {
                    ...
                    render:function(data, type, row, meta){
                        if(row.status == 1)
                            return '<span class="badge bg-success bg-gradient px-3 rounded-pill">Aktif</span>';
                        else
                            return '<span class="badge bg-danger bg-gradient px-3 rounded-pill">Tidak Aktif</span>';
                    },
                    width:"15%"
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        el.find('.edit_data,.delete_data,.view_data').attr('data-id',row.id).attr('data-name',row.nama_ekskul)
                        el.find('.edit_data').attr("href","./?page=ekskul/FormEkskul&id="+row.id)
                        el.find('.view_data').attr("href","./?page=ekskul/DetailEkskul&id="+row.id)
            ...
            drawCallback: function(settings) {
                ...
                $('.delete_data').click(function(){
                    _conf("Apakah Kamu Yakin Ingin Menghapus  <b>"+$(this).attr('data-name')+"</b> Dari List?","hapusEkskul",[$(this).attr('data-id')])
                })
            },
            language:{
                oPaginate: {
